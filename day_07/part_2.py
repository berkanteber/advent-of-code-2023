...
class Hand:
    ...
    def score(self) -> tuple[int, ...]:
        without_j = [c for c in self.cards if c != 'J']
        len_j = len(self.cards) - len(without_j)

        sums = [count for _, count in Counter(without_j).most_common()]
        if sums:
            sums[0] += len_j
        else:
            sums = [5]
...
print(solve(INPUT_DATA))  # 251224870
