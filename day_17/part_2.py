...
def solve(data: str) -> int:
    ...
    while nexts:
        ...
        for ppos, pdirs in possible_nexts:
            ...
            if len(set(pdirs)) == 2:
                if not pdirs.startswith(('r' * 4, 'l' * 4, 'u' * 4, 'd' * 4)):
                    continue
                else:
                    pdirs = pdirs[-1]

            if pdirs in ('r' * 11, 'l' * 11, 'u' * 11, 'd' * 11):
                continue
    ...
    return min(
        scost for (spos, sdirs), scost in seen.items()
        if spos == (max_row, max_col) and
        sdirs.startswith(('r' * 4, 'l' * 4, 'u' * 4, 'd' * 4))
    )
...
assert solve(TEST_DATA_1) == 94
assert solve(TEST_DATA_2) == 71

print(solve(INPUT_DATA))  # 925
